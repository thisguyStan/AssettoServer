...
public class AiBehavior : CriticalBackgroundService, IAssettoServerAutostart
{
    ...
    public AiBehavior(SessionManager sessionManager,
        ...
        AiSpline spline, AiParams aiParams) : base(applicationLifetime)
    {
        ...
        _aiParams.PropertyChanged += (_, _) => AdjustOverbooking();
    }
    ...
    private void Update()
    {
        ...
        foreach (var entryCar in _entryCarManager.EntryCars)
        {
            ...
            if (!entryCar.AiControlled
                ...
                && _sessionManager.ServerTimeMilliseconds - entryCar.LastActiveTime < _aiParams.PlayerAfkTimeoutMilliseconds
                && (_aiParams.TwoWayTraffic || _aiParams.WrongWayTraffic || drivingTheRightWay))
            {
        ...
        for (int i = 0; i < _initializedAiStates.Count; i++)
        {
            for (int j = 0; j < _playerCars.Count; j++)
            {
                if (_playerOffsetPositions.Count <= j)
                {
                    ...
                    {
                        offsetPosition += Vector3.Normalize(_playerCars[j].Status.Velocity) * _aiParams.PlayerPositionOffsetMeters;
                    }
        ...
        {
            if (dist.Value > _aiParams.PlayerRadiusSquared
    ...
    {
        using var timer = new PeriodicTimer(TimeSpan.FromMilliseconds(_aiParams.AiBehaviorUpdateIntervalMilliseconds));
    ...
    private bool IsPositionSafe(int pointId)
    {
        ...
        for (var i = 0; i < _entryCarManager.EntryCars.Length; i++)
        {
            ...
            if (entryCar.Client?.HasSentFirstUpdate == true
                && Vector3.DistanceSquared(entryCar.Status.Position, ops.Points[pointId].Position) < _aiParams.SpawnSafetyDistanceToPlayerSquared)
            {
                return false;
            }
        }
    ...
    private int GetSpawnPoint(EntryCar playerCar)
    {
        ...
        if (result.DistanceSquared > _aiParams.MaxPlayerDistanceToAiSplineSquared)
        {
            return -1;
        }
        
        int spawnDistance = Random.Shared.Next(_aiParams.MinSpawnDistancePoints, _aiParams.MaxSpawnDistancePoints);
    ...
    private void AdjustOverbooking()
    {
        ...
        int targetAiCount = Math.Min(playerCount * Math.Min((int)Math.Round(_aiParams.AiPerPlayerTargetCount * _aiParams.TrafficDensity), aiSlots.Count), _aiParams.MaxAiTargetCount);
