...
public class AiState
{
    ...
    {
        float variation = _aiParams.MaxSpeedMs * _aiParams.MaxSpeedVariationPercent;

        float fastLaneOffset = 0;
        if (_spline.Points[CurrentSplinePointId].LeftId >= 0)
        {
            fastLaneOffset = _aiParams.RightLaneOffsetMs;
        }
        InitialMaxSpeed = _aiParams.MaxSpeedMs + fastLaneOffset - (variation / 2) + (float)Random.Shared.NextDouble() * variation;
    ...
    public void Teleport(int pointId)
    {
        ...
        var minDist = _aiParams.MinAiSafetyDistanceSquared;
        var maxDist = _aiParams.MaxAiSafetyDistanceSquared;
        if (_aiParams.LaneCountSpecificOverrides.TryGetValue(_spline.GetLanes(CurrentSplinePointId).Length, out var overrides))
        {
        ...
        SafetyDistanceSquared = Random.Shared.Next((int)Math.Round(minDist * (1.0f / _aiParams.TrafficDensity)),
            (int)Math.Round(maxDist * (1.0f / _aiParams.TrafficDensity)));
    ...
    {
        if (_aiParams.IgnorePlayerObstacleSpheres != null)
        {
            foreach (var sphere in _aiParams.IgnorePlayerObstacleSpheres)
            {
    ...
    public void DetectObstacles()
    {
        ...
        else if (_stoppedForObstacle && _sessionManager.ServerTimeMilliseconds - _stoppedForObstacleSince > _aiParams.IgnoreObstaclesAfterMilliseconds)
        {
    ...
    public void Update()
    {
        ...
        Status.EngineRpm = (ushort)MathUtils.Lerp(EntryCar.AiIdleEngineRpm, EntryCar.AiMaxEngineRpm, CurrentSpeed / _aiParams.MaxSpeedMs);
